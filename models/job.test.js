...
describe("Job.findAllWithFilter() tests", function () {
  ...
  test("filter all jobs by minimum salary", async function () {
    const results = await Job.findAllWithFilter({ minSalary: 3 })
    expect(results).toEqual([{
      id: 3,
      title: "j3"/*,
      salary: 3,
  ...
  test("filter all jobs with hasEquity = false; meaning no filter", async function () {
    //Find all jobs with minimum equity of 1
...
describe("Job.create() tests", function () {
  test("creates a job correctly", async function () {
    const correctJobData = { id: 4, title: 'j4', salary: 4, equity: "0.4", company_handle: 'c3' }
    const newJob = await Job.create({ title: 'j4', salary: 4, equity: 0.4, company_handle: 'c3' })

    //Check the returned job is correct
    expect(newJob).toEqual(correctJobData)

    // Check the created job is correct in database
    const res = await db.query('SELECT * FROM jobs WHERE title = $1', ['j4'])
  ...
  test("fails when try to create a duplicate job", async function () {
    try {
      ...
      const newJob = await Job.create({ title: 'j1', salary: 1, equity: 0.1, company_handle: 'c1' })
      fail('No error was thrown, test failed.')
    }
    catch (err) {
      expect(err).toBeTruthy()
...
describe("Job.update() tests", function () {
  test("updates correctly", async function () {
    const updatedJob = await Job.update(1, { title: 'updated_j1', salary: 10, equity: 0.10 })
